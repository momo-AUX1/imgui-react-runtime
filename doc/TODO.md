...
- Add host components for core ImGui primitives still missing (TreeNode/Tree, TabBar/TabItem, ListBox, InputTextMultiline, InputScalar/Double variants, Image/ImageButton, Plot/Histogram helpers, Tooltip/Popup, DockSpace)
- Provide window modifiers for docking, background alpha, rounding, taskbar visibility, and explicit close-button toggles separate from callbacks
- Extend styling system beyond initial color/size support to cover padding, border radius, fonts, and global theme overrides
- Add keyboard navigation helpers (focus management, shortcut APIs) and expose ImGui IO flags for fine-grained control
