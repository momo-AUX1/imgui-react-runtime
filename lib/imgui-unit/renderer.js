...
function getStyleFromProps(props: any): any {
  if (!props) {
    return null;
  }
...
function getStyleColor(style: any, key: string): any {
  if (!style) {
    return undefined;
  }
...
function getStyleNumber(style: any, key: string) {
  if (!style) {
    return undefined;
  }
...
function pushFrameBgColorsFromStyle(style: any, vec4: c_ptr): number {
...
function pushHeaderColorsFromStyle(style: any, vec4: c_ptr): number {
  const colorValue = getStyleColor(style, "backgroundColor");
  if (colorValue === undefined) {
    return 0;
  }
...
function pushButtonColorsFromStyle(style: any, vec4: c_ptr): number {
  const colorValue = getStyleColor(style, "backgroundColor");
  if (colorValue === undefined) {
    return 0;
  }
...
function pushTextColorFromStyle(style: any, vec4: c_ptr): number {
...
function readUtf8String(ptr: c_ptr, maxBytes: number): string {
  if (ptr === 0 || maxBytes <= 0) {
    return "";
  }
...
function truncateStringToMaxLength(value: string, maxLength: number): string {
  if (maxLength <= 0) {
    return "";
  }
...
function validateInteger(value: any, defaultValue: number, propName: string): number {
...
function gatherInlineText(node: any, componentName: string): string {
  if (!node.children) {
    return "";
  }
  ...
  for (let i = 0; i < node.children.length; i++) {
    ...
    if (child.text !== undefined) {
      ...
      logErrorMessage(
        `<${componentName}> only supports text children. Ignoring <${child.type}>.`
      );
    }
  }
...
function renderMenuBar(node: any): void {
  if (!_igBeginMenuBar()) {
    return;
  }
...
function renderMainMenuBar(node: any): void {
...
function renderMenu(node: any): void {
  const props = node.props;
...
function renderMenuItem(node: any): void {
  const props = node.props;
  ...
  if (hasToggle) {
    ...
    if (props && props.selected === undefined && node._menuItemSelected !== newValue) {
 ...
 */
function renderButton(node: any, vec2: c_ptr, vec4: c_ptr): void {
...
function renderCheckbox(node: any, vec4: c_ptr): void {
  const props = node.props;
  ...
  if (!hasControlledValue && (changed || node._checkboxValue !== actualValue)) {
...
function renderSliderFloat(node: any): void {
  const props = node.props;
...
function renderSliderInt(node: any): void {
  const props = node.props;
...
function renderProgressBar(node: any, vec2: c_ptr): void {
  const props = node.props;
...
function renderInputText(node: any): void {
  const props = node.props;
...
function renderInputFloat(node: any): void {
  const props = node.props;
...
function renderInputInt(node: any): void {
  const props = node.props;
...
function renderDragFloat(node: any): void {
  const props = node.props;
...
function renderDragInt(node: any): void {
  const props = node.props;
...
function renderCombo(node: any): void {
  const props = node.props;
  ...
  if (hasControlledIndex && props) {
    currentIndex = validateInteger(props.selectedIndex, 0, "combo selectedIndex");
  } else {
  ...
  if (!hasControlledIndex && node._comboIndex !== newIndex) {
...
function renderSelectable(node: any, vec2: c_ptr, vec4: c_ptr): void {
  const props = node.props;
  ...
  if (!hasControlledValue && node._selectableSelected !== newValue) {
...
function renderRadioButton(node: any, vec4: c_ptr): void {
  const props = node.props;
  ...
  if (hasGroupControl && props) {
    isSelected = props.selectedValue === props.value;
  ...
  if (pressed) {
    if (!hasGroupControl && !hasBoolControl) {
...
function readColorComponent(ptr: c_ptr, offset: number): number {
...
function renderColorEdit3(node: any): void {
  const props = node.props;
...
function renderColorEdit4(node: any): void {
  const props = node.props;
...
function renderColorButton(node: any, vec2: c_ptr): void {
  const props = node.props;
...
function renderSpacing(node: any): void {
  const props = node.props;
