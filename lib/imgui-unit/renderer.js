...
function safeInvokeCallback(callback, ...args) {
  ...
  try {
    const flushSync = globalThis && globalThis.__reactImguiFlushSync;
    const discreteUpdates = globalThis && globalThis.__reactImguiDiscreteUpdates;
    const batchedUpdates = globalThis && globalThis.__reactImguiBatchedUpdates;

    if (typeof flushSync === 'function') {
      flushSync(invoke);
    } else if (typeof discreteUpdates === 'function') {
      discreteUpdates(invoke);
    } else if (typeof batchedUpdates === 'function') {
      batchedUpdates(invoke);
    } else {
      invoke();
    }
  } catch (e) {
...
function renderMenuItem(node) {
  ...
  if (hasToggle) {
    const pointer = allocTmp(SIZEOF_C_BOOL);
    let currentValue;
    const propsVersion = (node._propsVersion >>> 0) || 0;

    if (props && props.selected !== undefined) {
      const controlledValue = !!props.selected;
      const pendingValue = node._pendingControlledMenuItem;
      const pendingVersion = (node._pendingControlledMenuItemVersion >>> 0) || 0;
      if (pendingValue !== undefined && pendingVersion === propsVersion && pendingValue !== controlledValue) {
        currentValue = pendingValue;
      } else {
        node._pendingControlledMenuItem = undefined;
        node._pendingControlledMenuItemVersion = undefined;
    ...
    if (props && props.selected !== undefined) {
      ...
      node._pendingControlledMenuItemVersion = propsVersion;
...
function renderCheckbox(node, vec4) {
  ...
  if (hasControlledValue && props) {
    ...
    const propsVersion = (node._propsVersion >>> 0) || 0;
    const pendingVersion = (node._pendingControlledValueVersion >>> 0) || 0;
    if (pendingValue !== undefined && pendingVersion === propsVersion && pendingValue !== controlledValue) {
      currentValue = pendingValue;
    } else {
      node._pendingControlledValue = undefined;
      node._pendingControlledValueVersion = undefined;
      currentValue = controlledValue;
    }
  } else {
  ...
  if (hasControlledValue) {
    ...
    node._pendingControlledValueVersion = (node._propsVersion >>> 0) || 0;
...
function renderCombo(node) {
  ...
  if (hasControlledIndex && props) {
    ...
    const propsVersion = (node._propsVersion >>> 0) || 0;
    const pendingVersion = (node._pendingControlledComboIndexVersion >>> 0) || 0;
    if (pendingIndex !== undefined) {
      const clampedPending = itemCount > 0
        ? Math.max(0, Math.min(pendingIndex | 0, itemCount - 1))
        : 0;
      if (pendingVersion === propsVersion && clampedPending !== controlledIndex) {
        currentIndex = clampedPending;
      } else {
        node._pendingControlledComboIndex = undefined;
        node._pendingControlledComboIndexVersion = undefined;
  ...
  if (hasControlledIndex) {
    if (changed) {
      ...
      node._pendingControlledComboIndexVersion = (node._propsVersion >>> 0) || 0;
    }
...
function renderSelectable(node, vec2, vec4) {
  ...
  if (hasControlledValue && props) {
    controlledValue = !!props.selected;
    const pendingValue = node._pendingControlledSelected;
    const propsVersion = (node._propsVersion >>> 0) || 0;
    const pendingVersion = (node._pendingControlledSelectedVersion >>> 0) || 0;
    if (pendingValue !== undefined) {
      if (pendingVersion === propsVersion && pendingValue !== controlledValue) {
        currentValue = pendingValue;
      } else {
        node._pendingControlledSelected = undefined;
        node._pendingControlledSelectedVersion = undefined;
        currentValue = controlledValue;
      }
    } else {
      currentValue = controlledValue;
  ...
  if (hasControlledValue) {
    if (newValue !== controlledValue) {
      ...
      node._pendingControlledSelectedVersion = (node._propsVersion >>> 0) || 0;
    } else if (node._pendingControlledSelected !== undefined) {
      node._pendingControlledSelected = undefined;
      node._pendingControlledSelectedVersion = undefined;
    }
