...
function renderTreeNodeComponent(node) {
  ...
  if (opened) {
    try {
      ...
      if (!(flags & _ImGuiTreeNodeFlags_NoTreePushOnOpen)) {
        _igTreePop();
      }
    }
  }
...
function renderTabItem(node) {
  ...
  const wasActive = !!node._tabItemWasActive;
  let stillOpen = true;
...
function renderListBox(node, vec2) {
  ...
  if (hasControlledIndex && props) {
    currentIndex = validateInteger(props.selectedIndex, 0, "listbox selectedIndex");
  } else {
  ...
  if (!hasControlledIndex && node._listBoxIndex !== clampedIndex) {
...
function renderPopupModal(node) {
  ...
  if (wantsCloseCallback) {
    ...
    if (!stillOpen && isOpenNow) {
      _igCloseCurrentPopup();
      isOpenNow = false;
    }
    if (!stillOpen && props && props.onClose && !node._popupModalCloseNotified) {
      safeInvokeCallback(props.onClose);
