...
(function () {
  ...
  globalThis.console = {
    ...
    const COLOR_SCHEME_UNKNOWN = 'unknown';
    const validColorSchemes = new Set(['light', 'dark']);
    const colorSchemeListeners = new Set();
    let currentColorScheme = COLOR_SCHEME_UNKNOWN;
    ...
    function normalizeColorScheme(value) {
        ...
        return validColorSchemes.has(normalized) ? normalized : COLOR_SCHEME_UNKNOWN;
    }

    function emitColorSchemeChange() {
        colorSchemeListeners.forEach((listener) => {
            try {
                listener(currentColorScheme);
            } catch (error) {
                if (globalThis.console && typeof globalThis.console.error === 'function') {
                    globalThis.console.error('Appearance listener error', error);
                }
            }
        });
    }

    globalThis.__colorScheme = currentColorScheme;

    globalThis.__setColorScheme = function(scheme) {
        const normalized = normalizeColorScheme(scheme);
        if (normalized === currentColorScheme) {
            return;
        }
        currentColorScheme = normalized;
        globalThis.__colorScheme = currentColorScheme;
        emitColorSchemeChange();
    };

    globalThis.__getColorScheme = function() {
        return currentColorScheme;
    };

    globalThis.__registerColorSchemeListener = function(listener) {
        if (typeof listener !== 'function') {
            return function noop() {};
        }
    ...
    globalThis.__unregisterColorSchemeListener = function(listener) {
        colorSchemeListeners.delete(listener);
    };
